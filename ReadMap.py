...
class MapReading():
    ...
    def Get_position_line():
        ...
        if first_endlocation_name[-1] == "\""   :  #如果名称中最后一位是 “号, 则把它去掉
            ...
            print("ID: ",list(map(int,number)))
        end_pos_x = map1[first_endPos_num+39:first_endPos_num+50]#获取站点的X坐标名称
        end_Pos_y = map1[first_endPos_num+50:first_endPos_num+61]#获取站点的y坐标名称
        ...
        next_startPos_num = int(first_startPos_num)
        next_endPos_num = int(first_endPos_num)
        while next_endPos_num < len(map1) :#输出所有startPos和endpos的位置#
            try:     #输出下一起点名称和坐标
                next_startPos_num = map1.index(str_startPos, next_endPos_num+1 )#从next_endPos_num位置开始往下检索
                print ("next_startPos_num: ", next_startPos_num)
                next_startlocation_name = map1[next_startPos_num+25:next_startPos_num+31]#获取下一个站点的字符名称
                if next_startlocation_name[-1] == "\""   :  
                    ...
                    print("名称字符中的数字:", list(map(int,number)))
                next_start_pos_x = map1[next_startPos_num+39:next_startPos_num+50]#获取站点的X坐标名称
                next_start_pos_y = map1[next_startPos_num+50:next_startPos_num+61]#获取站点的y坐标名称
            ...
            try:     #输出下一终点名称和坐标
                next_endPos_num = map1.index(str_endPos, next_startPos_num+1 )#从next_startPos_num位置开始往后检索
